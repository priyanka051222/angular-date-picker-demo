...
export class DemoComponent {
  ...
  configChanged(change = 'N/A', value: any = 'N/A') {
  ...
  log(item) {
    ...
    if (item) {
        if (moment(item).format('MMM Do YY') === moment(today).format('MMM Do YY')) {
        this.formatDate = `Today ${moment(item).format('LL')}`;
     } else {
        this.formatDate =   `${moment(item).format('dddd')} ${moment(item).format('LL')}`;
     }
    }
  }
